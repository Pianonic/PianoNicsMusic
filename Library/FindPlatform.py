...
from Library.SourceGetter import SoundCloudLoaderRequest, SpotifyLoaderRequest, YoutubeLoader, TikTokLoaderRequest
from Library import GenerateEmbed
...
async def find_platform(ctx, query):
    ...
    if "youtube.com" in query or "youtu.be" in query:
        return await handle_platform(ctx, query, "YouTube")
    
    if "https://soundcloud.com" in query:
        return await handle_platform(ctx, query, "SoundCloud")
...
async def handle_attachment(ctx):
    ...
    return ReturnData(Data(link=link, song_name=filename, author=ctx.author.mention, image="https://i.giphy.com/LNOZoHMI16ydtQ8bGG.webp"), message)
...
async def send_loading_message(ctx, text):
    try:
        if ctx.message:
            return await ctx.send(embed=GenerateEmbed.CreateEmbed("⏳ Please Wait ⏳", text + " ⌚"))
        else:
            return await ctx.respond(embed=GenerateEmbed.CreateEmbed("⏳ Please Wait ⏳", text + " ⌚"))
    except:
        return await ctx.send(embed=GenerateEmbed.CreateEmbed("⏳ Please Wait ⏳", text + " ⌚"))

async def handle_youtube(ctx, query, message):
    return ReturnData(await YoutubeLoader.GetYTLink(query), message)

async def handle_soundcloud(ctx, query, message):
    return ReturnData(await SoundCloudLoaderRequest.GetSCLink(query), message)

async def handle_spotify(ctx, query, message):
    return ReturnData(await SpotifyLoaderRequest.GetSFLinkRequest(query), message)

async def handle_tiktok(ctx, query, message):
    return ReturnData(await TikTokLoaderRequest.GetTTLink(query), message)
...
async def handle_url(ctx, query):
    ...
    return ReturnData(Data(link=query, song_name=filename, author=ctx.author.mention, image="https://i.giphy.com/LNOZoHMI16ydtQ8bGG.webp"), message)
...
async def handle_search(ctx, query):
    ...
    return ReturnData(await YoutubeLoader.GetYTLink(video_url), message)
